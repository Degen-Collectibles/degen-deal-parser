...
def catalog_details(product):
    ...
    with httpx.Client(timeout=12, headers=TCGTRACKING_HEADERS) as client:
        ...
        return {'description': plain((raw.get('ext_data') or {}).get('CardText'))[:6000],
...
def unit_key(name):
    ...
    counts = tuple(re.findall(r'\b\d+\s*[- ]?\s*(?:packs?|boxes|count|ct)\b|\bset of \d+\b', normalized))
    return (kind, modifiers, counts)
...
def suggestions(product, fields, details, listing, metadata):
    ...
    market = None
    if product.get('market_price_source') == 'TCGPlayer Market' and lang == language(product):
        try:
    ...
    if listing and comparable(product, {**fields, 'language': lang}, listing):
        ...
        if category in cats:
            ...
            proposed['attributes'] = {str(a['id']): str(a['values'][0].get('name') or '') for a in listing.get('product_attributes', []) if a.get('values')}
...
def apply_defaults(draft, proposed, sources, details, warning=''):
    ...
    for key, value in proposed.items():
        if not fields.get(key) or fields.get(key) == previous.get(key):
            fields[key] = value
    draft['defaults'] = {'values': proposed, 'sources': sources, 'warning': warning}
