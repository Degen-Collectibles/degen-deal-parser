...
def test_finance_bank_data_excludes_credit_card_payments_from_expenses():
    engine = create_engine("sqlite://", connect_args={"check_same_thread": False})
    SQLModel.metadata.create_all(engine)
    posted_at = datetime(2026, 5, 12, 12, tzinfo=timezone.utc)
    ...
    with Session(engine) as session:
        ...
        session.add(
            BankTransaction(
                ...
                account_type="checking",
                posted_at=posted_at,
                description="Payment to Chase card ending in 2024",
                amount=-1000.0,
                classification="transfer_or_card_payment",
    ...
    transfer_row = next(row for row in data["detail_rows"] if row["category"] == "transfers")
    category_keys = {row["category"] for row in data["category_rows"]}

    assert transfer_row["excluded_from_finance"] is True
    assert data["gross_outflow_total"] == 1080.0
    assert data["bank_only_total"] == 80.0
    assert data["finance_excluded_total"] == 1000.0
    assert data["non_operating_total"] == 0.0
    assert data["daily_rows"][0]["bank_only_total"] == 80.0
    assert data["daily_rows"][0]["non_operating"] == 0.0
