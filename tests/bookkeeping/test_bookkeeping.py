...
from app.finance.bookkeeping import fetch_google_sheet_export, read_tabular_rows, reconcile_bookkeeping_import
...
class GoogleSheetExportFetchTests(unittest.TestCase):
    def test_follows_safe_google_redirect_before_streaming_export(self):
        ...
        with patch("app.finance.bookkeeping.httpx.AsyncClient", return_value=client):
    ...
    def test_rejects_google_sheet_export_redirect_to_untrusted_host(self):
        ...
        export_url = "https://docs.google.com/spreadsheets/d/abc/export?format=xlsx"
        with patch("app.finance.bookkeeping.httpx.AsyncClient", return_value=FakeAsyncClient()):
            with self.assertRaises(ValueError):
